      ...
      <div className="bg-white border-b overflow-hidden relative">
        ...
        <div className="container mx-auto px-6 py-16 text-center">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto mb-10">
            Cada libro donado es una puerta que se abre para un nuevo lector.
            Nuestra comunidad se fortalece con cada contribución.
          </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loading ? (
                ...
                activeDonors.map((donor) => (
                  <Card key={donor.id} className="overflow-hidden border-none shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <tbody>
                      {recentDonations.map((donation) => (
                        <tr key={donation.id} className="bg-white border-b hover:bg-gray-50/50">
                          <td className="px-6 py-4 font-medium text-gray-900">{donation.bookTitle}</td>
                          <td className="px-6 py-4">{donation.bookAuthor}</td>
                          <td className="px-6 py-4">{donation.donor?.name || "Anónimo"}</td>
                          <td className="px-6 py-4 text-center">{new Date(donation.donationDate).toLocaleDateString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
